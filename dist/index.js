...
var CountryVaccination = /*#__PURE__*/function (_BaseChartComponent) {
  _inherits(CountryVaccination, _BaseChartComponent);

  var _super = _createSuper(CountryVaccination);

  function CountryVaccination() {
    var _this;

    _classCallCheck(this, CountryVaccination);
  ...
  _createClass(CountryVaccination, [{
    key: "draw",
    ...
    value: function draw() {
      ...
      var dateAxisFormat = d3.timeFormat(props.format.dateAxis);
      var formatNumber = d3.format(props.format.number);
      var getCountry = client.getCountry(props.countryISO);
      var pop = 0;
      ...
      var _node$getBoundingClie = node.getBoundingClientRect(),
          containerWidth = _node$getBoundingClie.width; // Respect the width of your container!
      ...
      if (props.dateRange.start) {
        ...
        startDate = d3.min(data, function (d) {
          return d.parsedDate;
        });
      }
      ...
      for (var i = popMilestones.length - 1; i >= 0; i--) {
      ...
      ms0.appendSelect('text').attr('transform', "translate(0,".concat(-10, ")")).style('fill', props.milestoneStyle.textFill).text(Mustache__default['default'].render(props.text.milestone, {
        number: useMilestonePer * 100
      }));

      if (milestoneIndex > 0) {
        var ms1 = milestoneG.appendSelect('g.milestone-1').attr('transform', "translate(0, ".concat(yScale(popMilestones[milestoneIndex - 1]), ")"));
        ms1.appendSelect('line').style('stroke', props.milestoneStyle.stroke).style('stroke-dasharray', props.milestoneStyle['stroke-dasharray']).attr('x1', 0).attr('x2', width).attr('y1', 0).attr('y2', 0);
...
module.exports = CountryVaccination;
